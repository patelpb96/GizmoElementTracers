...
@author: Andrew Emerick <aemerick11@gmail.com>, Andrew Wetzel <arwetzel@gmail.com>
...
class AgetracerClass(dict, ut.io.SayClass):
    ...
    def determine_tracer_info(self, directory='.'):
        ...
        -----------
        directory : str
            top-level work directory for files (optional). Default: '.'
        '''
        ...
        possible_paths = [
            ut.io.get_path(directory),
            ut.io.get_path(directory + 'gizmo'),
        ...
        if path_file_name is None:
            print(f'Cannot find GIZMO_config.h or gizmo.out in {directory} or in {directory}/gizmo')
    ...
    def generate_age_bins(self, file_name='age_bins.txt', directory='.'):
        ...
        directory : str
            Top-level work directory. Default : '.'
        '''
        ...
        if self._logbins:
            ...
            self.say(
                '* reading age bin information from :  {}\n'.format(path_file_name.strip('./')),
        ...
        if len(self.age_bins) > self.info['num_tracers'] + 1:
            raise RuntimeError(
                'Number of age bins implies there should be more tracers. Something is wrong here.'
            )
        elif len(self.age_bins) < self.info['num_tracers'] + 1:
            raise RuntimeError(
                'Number of age bins implies there should be less tracers. Something is wrong here'
            )
...
def read_agetracer_times(directory='.', filename='age_bins.txt', verbose=True):
    '''
    Within input directory, search for and read stellar age tracer field bin times (if custom sized
    age tracer fields used). Return as an array.
    ...
    Returns
    -------
    Agetracers : dictionary class
        age tracer information
    '''
    ...
    for i in np.arange(np.size(agebins) - 1):
        ...
        for j, e in enumerate(elements):
            yield_table[i][j] = integrate.quad(
                yield_object.yields, min_t, max_t, args=(e,), points=points
            )[0]
    return yield_table
...
class FIRE2_yields(YieldsObject):
    ...
    def __init__(self, name='FIRE2', model_Z=1.0, Z_scaling=True):
        ...
        self.model_parameters = {'model_Z': model_Z, 'Z_scaling': Z_scaling}
        ...
        for e in self.elements:
            self._star_massfraction[e] = (
                self.model_parameters['model_Z'] * gizmo_star.sun_massfraction[e]
            )
        ...
        # all
        self.integration_points = np.sort(
            [0.003401, 0.010370, 0.03753, 0.001, 0.05, 0.10, 1.0, 14.0]
        )
    ...
    def compute_yields(self, model_Z=None):
        ...
        self.snIa_yields = gizmo_star.get_nucleosynthetic_yields(
            'supernova.ia',
        ...
        self.snII_yields = gizmo_star.get_nucleosynthetic_yields(
            'supernova.ii',
            star_metallicity=self.model_parameters['model_Z'],
            star_massfraction=self._star_massfraction,
            normalize=False,
        ...
        #
        self.wind_yields = gizmo_star.get_nucleosynthetic_yields(
            'wind',
            star_metallicity=self.model_parameters['model_Z'],
            star_massfraction=self._star_massfraction,
            normalize=False,
    ...
    def yields(self, t, element):
        ...
        windrate = gizmo_star.StellarWind.get_rate(
            t * 1000.0, metallicity=self.model_parameters['model_Z']
        )

        y = (
            (self.wind_yields[element] * windrate)
            + (self.snIa_yields[element] * snIarate)
            + (self.snII_yields[element] * snIIrate)
        )  # in Msun / Myr
