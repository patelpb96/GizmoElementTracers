...
class ParticlePointerDictionaryClass(dict, ut.io.SayClass):
    ...
    def __init__(self, part_z0=None, part_z=None, species_names=['star', 'gas']):
        ...
        if part_z0 is not None and part_z is not None:
            ...
            for spec in species_names:
                ...
                if spec in part_z and len(part_z[spec][self.id_name]) > 0:
    ...
    def get_pointers(
        ...
        if forward:
            ...
            z_from = self.z_name
            z_to = z_ref_name
            if (species_name_from == 'star' and len(species_names_to) == 1
            ...
            z_from = z_ref_name
            z_to = self.z_name
            if (species_name_from == 'gas' and len(species_names_to) == 1
...
class ParticlePointerIOClass(ut.io.SayClass):
    ...
    def __init__(
        self, species_names=['star', 'gas'], track_directory=TRACK_DIRECTORY,
        snapshot_directory=SNAPSHOT_DIRECTORY, reference_snapshot_index=600):
        '''
        Parameters
        ----------
        species_names : str or list : name[s] of particle species to track
        track_directory : str : directory where of pointer files
    ...
    def io_pointers(
        self, part=None, snapshot_index=None, ParticlePointer=None, track_directory=None):
        '''
    ...
    def _write_pointers_to_snapshot(self, part_z0, snapshot_index, count_tot={}):
        ...
        if part_z0_total_indices.size != ParticlePointer[z + 'particle.number']:
            ...
            if (self.test_property and self.test_property != self.match_property
...
class ParticleCoordinateClass(ut.io.SayClass):
    ...
    def __init__(
        self, species_name='star', track_directory=TRACK_DIRECTORY,
        snapshot_directory=SNAPSHOT_DIRECTORY, reference_snapshot_index=600,
        ...
        track_directory : str : directory to write formation coordinate files
            ...
            for prop in dict_in:
                if prop == 'id':
                    ...
                    if np.ndim(dict_in[prop]) == 2:
                        dict_in[prop] = np.array([dict_in[prop]])  # deal with older track files
    ...
    def _write_formation_coordinates(
        ...
        if part_z0_indices.size > 0:
            ...
            self.GizmoRead.assign_host_coordinates(
                part_z, self.species_name, hosts_part_z_indicess, host_number,
